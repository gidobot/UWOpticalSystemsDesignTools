...
class CameraResponseExperiments():
    ...
    def __init__(self):
        ...
        path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "../cfg/icx285AL.json")
    ...
    def test_camera_response(self, relative_irradiance_path, exposure_time):
        ...
        idx = np.argmax(i)
        irradiance_filter = np.zeros(len(i))

        delta = 1
        irradiance_filter[idx-delta:idx+delta+1] = i[idx-delta:idx+delta+1]
        area = np.trapz(irradiance_filter, w)
        #plt.plot(w, irradiance_filter)
        #plt.show()
        digital_signal_broad = self.camera.sensor.compute_digital_signal_broadband(exposure_time, w, i)
        digital_signal_narrow = self.camera.sensor.compute_digital_signal(exposure_time, w[idx], area)
...
if __name__ == '__main__':
    ...
    for i in range(len(data_list)):
        ...
        response_broad, response_narrow, exposure = experiments.test_camera_response(file, exposure_time)
